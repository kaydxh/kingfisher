...
int FilePraser::Load(const std::string &file) {
  ...
  bool FilePraser::isComment(std::string & str) {
    if (str.length() == 0) {
      return true;
  ...
  void FilePraser::trim(std::string & str) {
    trimRight(str);
    trimLeft(str);
  }

  void FilePraser::trimLeft(std::string & str, char c) {
    int i = 0;
    int len = str.length();
  ...
  void FilePraser::trimRight(std::string & str, char c) {
    int i = 0;
    int len = str.length();
  ...
  bool FilePraser::parser(const std::string &content, std::string &key,
                          std::string &value) {
    int i = 0;
