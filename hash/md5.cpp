//
// Created by kayxhding on 2020-04-15 14:07:28
//
...
std::string MD5::hexdigest(const MD5Digest &digest) const {
  char buf[33];
  for (int i = 0; i < 16; i++) {
    sprintf(buf + i * 2, "%02x", digest.digest[i]);
  }
  buf[32] = 0;
...
void MD5::md5Sum(const std::string &str, MD5Digest &digest) {
  Context ctx;
  init(&ctx);
  update(&ctx, str.c_str(), str.length());
  finalize(&ctx, &digest);
}

std::string MD5::Md5SumString(const std::string &str) {
  MD5Digest digest;
  md5Sum(str, digest);
...
void MD5::update(Context *ctx, const unsigned char *input, size_t length) {
  ...
  if ((ctx->count[0] += (length << 3)) < (length << 3)) ctx->count[1]++;
...
void MD5::finalize(Context *ctx, MD5Digest *digest) {
  ...
  encode(digest->digest, ctx->state, 16);
