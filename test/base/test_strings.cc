...
// ./bin/kingfisher_base_test --gtest_filter=test_Strings.WStringFromBytes
TEST_F(test_Strings, WStringFromBytes) {
    std::string str = "你好";
    std::wstring wstr;
    int ret = WStringFromBytes(wstr, str);
    ASSERT_TRUE(ret == 0);
    ASSERT_TRUE(wstr == L"你好");

    std::cout << "str: " << str << std::endl;
    std::wcout << L"wstr: " << L"你好" << wstr << std::endl;
}

TEST_F(test_Strings, WStringToBytes) {
    std::string str;
    std::wstring wstr = L"你好";
    int ret = WStringToBytes(str, wstr);
    ASSERT_TRUE(ret == 0);
    ASSERT_TRUE(str == "你好");
